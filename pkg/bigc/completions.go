...
const (
	...
	CATEGORY_KEYWORD_GENERATOR_PROMPT  = `You are a search engine optimisation specialist.\nGiven the name of a product category, generate a list of search keywords that a consumer may use to find the category via Google.\nExample:\nCategory Name: First Aid & Sport\nKeywords: first aid, wound care, sports, sports performance, burns, bites, hydration, hot packs, cold pack, aching muscles, cuts, bruises, antiseptic, bandages, band aids, plasters, strapping, tape, gauze, dressings`
	CATEGORY_METADATA_GENERATOR_PROMPT = `You are a search engine optimisation specialist.\nGiven the name of a product category, generate a meta description of the category for the metadata of a shop webpage to assist in search engine optimisation.\nExample:\nCategory Name: Beauty Tools\nMetadata description: Find the perfect exfoliators and manicure sets to keep yourself feeling beautiful.\nDiscover our variety of skin and body care products to maintain glowing skin.`
)
...
func (c AI_Client) GenerateCategoryMetaFromCategoryName(categoryName string, model string) (string, error) {
	if model == "" {
		model = openai.GPT3Dot5Turbo
	}
	m := []openai.ChatCompletionMessage{
		{
			Role:    openai.ChatMessageRoleSystem,
			Content: CATEGORY_METADATA_GENERATOR_PROMPT,
		},
		{
			Role:    openai.ChatMessageRoleUser,
...
func (c AI_Client) GenerateCategoryKeywordsFromCategoryName(categoryName string, model string) (string, error) {
	if model == "" {
		model = openai.GPT3Dot5Turbo
	}
	m := []openai.ChatCompletionMessage{
		{
			Role:    openai.ChatMessageRoleSystem,
			Content: CATEGORY_KEYWORD_GENERATOR_PROMPT,
		},
		{
			Role:    openai.ChatMessageRoleUser,
			Content: categoryName,
